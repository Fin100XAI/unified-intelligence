// ============================================================================
// Simulated Maharashtra Governance Data Generator  —  PoC / DEMO ONLY
// NOTE: All numbers below are SYNTHETIC and generated for demonstration.
// This is NOT real government data. Do not treat as official statistics.
// ============================================================================

// --- Seeded pseudo-random. Reseed each request so live values drift over time -
let _seed = 42;
export const reseed = (s) => { _seed = ((Math.floor(s) % 233280) + 233280) % 233280 || 1; };
const rnd = () => {
...
// Real-world anchors for major Maharashtra schemes (approx. public figures,
// FY 2024-25 / 2025-26): eligible beneficiaries + annual outlay in ₹ crore.
// Numbers are grounded in published data; small live drift is applied at runtime.
export const SCHEME_ANCHORS = {
...
// Small ± drift around a real anchor so figures feel live without losing realism.
...
export const buildDistricts = () =>
  flatDistricts().map(({ name, division }) => {
    ...
    return {
      ...
      division,
      governanceScore: gov,
      welfareScore: r(45, 95),
      rtsBacklog: r(120, 4200),
      revenueRiskPct: rf(2, 28),
      disasterVulnerability: r(20, 90),
      lawOrderLevel: pick(RISK),
      infraDelayRisk: r(10, 80),
...
export const buildDepartments = () =>
  DEPARTMENTS.map((name) => {
    ...
    return {
      department: name,
      healthScore: health,
      pendingApprovals: r(80, 5200),
      slaBreaches: r(20, 1400),
      budgetUtilizationPct: rf(38, 99),
      grievanceLoad: r(150, 8000),
      riskScore: 100 - health,
      riskLevel: riskFromScore(health),
      trend: months.map((m) => ({ month: m, score: r(45, 95) })),
    };
  });

// --- Overview -----------------------------------------------------------------
export const buildOverview = (districts, departments) => {
  const escalations = districts
    .map((d) => ({
      id: 'ESC-' + r(1000, 9999),
      title: pick([
        'RTS SLA breach surge', 'DBT disbursement stall', 'Flood readiness gap',
  ...
  return {
    governanceHealthScore: 72,
    departmentsMonitored: departments.length,
    districtsMonitored: districts.length,
    criticalAlerts: r(28, 64),
    rtsPending: r(180000, 240000),
    welfareGapPct: rf(8, 22),
    disasterRiskDistricts: districts.filter((d) => d.disasterVulnerability > 65).length,
    // Maharashtra FY2025-26 own revenue receipts ≈ ₹4.9 lakh crore.
    revenueCollectionCr: drift(395000, 0.04), // ₹ crore (~80% of target)
    revenueTargetCr: 490000,
    topEscalations: escalations,
    deptRanking: departments
      .map((d) => ({ department: d.department, healthScore: d.healthScore }))
...
export const buildWelfare = () => {
  ...
  return {
    ...
    dbtDelayRiskPct: rf(6, 14),
    schemes,
    byDistrict: flatDistricts().map(({ name, division }) => ({
      district: name, division, welfareGapPct: rf(4, 28),
    })),
  };
};

// --- RTS / Aaple Sarkar -------------------------------------------------------
export const buildRTS = () => ({
  total: 2840000, pending: r(190000, 240000), slaBreached: r(34000, 62000),
  avgDisposalDays: rf(3, 14),
  ...
  highRiskServices: RTS_SERVICES.map((s) => ({ service: s, breachRiskPct: rf(8, 45) })).sort((a, b) => b.breachRiskPct - a.breachRiskPct).slice(0, 5),
});

// --- MahaDBT ------------------------------------------------------------------
export const buildMahaDBT = () => {
  ...
  const totalDisbursedCr = bySchemeFlow.reduce((t, x) => t + x.disbursedCr, 0);
  return {
    totalDisbursedCr, pendingDisbursedCr: bySchemeFlow.reduce((t, x) => t + x.pendingCr, 0),
    failedTxns: r(42000, 98000), bankLinkingIssues: r(120000, 280000),
    duplicateRiskFlags: r(3000, 12000), verifiedPct: rf(88, 96),
    bySchemeFlow,
...
export const buildRevenue = () => ({
  targetCr: 460000, collectedCr: r(300000, 420000),
...
// --- Disaster -----------------------------------------------------------------
export const buildDisaster = () => ({
  vulnerabilityIndex: r(45, 72),
  risks: { flood: r(40, 90), heatwave: r(30, 85), drought: r(25, 80), coastal: r(35, 88), urbanFlooding: r(40, 92) },
  byDistrict: flatDistricts().map(({ name, division }) => ({
    district: name, division, readinessScore: r(40, 95),
    floodRisk: r(20, 95), reliefResourcesPct: rf(45, 98),
  })),
...
// --- Law & Order (governance-safe, NO surveillance) ---------------------------
export const buildLawOrder = () => ({
  ...
  byDistrict: flatDistricts().map(({ name }) => ({
    district: name, incidentConcentration: r(10, 100), responseStatus: pick(['Resolved', 'In Progress', 'Coordinating']),
    level: pick(RISK),
  })),
...
// --- Cyber Governance & AI Trust ----------------------------------------------
export const buildCyber = () => ({
  cyberReadinessScore: r(58, 84),
  certInAlignmentPct: rf(70, 95), dataProtectionPct: rf(72, 96),
...
// --- AI Governance Alerts & Audit Logs ----------------------------------------
export const buildAuditLogs = () =>
  Array.from({ length: 24 }, (_, i) => ({
    id: 'AI-LOG-' + (1000 + i),
    recommendation: pick([
      'Prioritize RTS backlog clearance in Beed', 'Flag DBT duplicate cluster in Nanded',
    ...
    explainability: pick([
      'Backlog 2.3x division avg; SLA breach trend rising',
...
export const buildAlerts = (districts) =>
  Array.from({ length: 12 }, () => {
    const d = pick(districts);
    return {
      id: 'ALERT-' + r(10000, 99999), title: pick([
        'AI Governance Risk: anomalous spend', 'SLA breach risk threshold crossed',
      ...
      priority: pick(RISK), risk: pick(RISK), explainable: true,
      time: r(1, 240) + ' min ago',
    };
  });
